...
class SourceFile(Base):
    ...
    format: Mapped[str] = mapped_column(String(30), primary_key=True)
    
    image_url: Mapped[str] = mapped_column(String(120))
    
    status: Mapped[str] = mapped_column(Enum(SourceFileStatus))
    preprocessed_bands: Mapped[Optional[List["PreprocessedBand"]]] = relationship("PreprocessedBand", default_factory=list)
...
class PreprocessedBand(Base):
    ...
    main_id: Mapped[str] = mapped_column(ForeignKey("capture_data.main_id"), primary_key=True)
    band_id: Mapped[str] = mapped_column(ForeignKey("source_file.file_id"), primary_key=True)
...
class EGProcessor:
    ...
    def get_source_file_data(self):
        ...
        blob_finder = re.compile(r"^(?:tiles.*?IMG_DATA.*?|L.*?)_(?P<file_id>B[0-9A]{1,2}|MTL)\.(?P<format>TIF|jp2|txt)$")
        ...
        try:
            for captured_data_element in session.query(CaptureData).all():
                ...
                if len(captured_data_element.source_files) < len(self.bands_definition[mission]):
                    ...
                    bucket = storage.Bucket(client=self.storage_client, name=parsed_url['bucket_name'])
                                        
                    for blob in bucket.list_blobs(prefix=parsed_url['blobs_path_name'], retry=self.retry_methodology):
                        if test := blob_finder.search(blob.name):
                            ...
                            if session.query(SourceFile).where(SourceFile.main_id == captured_data_element.main_id).where(SourceFile.file_id == file_id).count() > 0:
                                ...
                                continue
                            captured_data_element.add_source_file(
                                SourceFile(main_id=captured_data_element.main_id,
                                           file_id= file_id,
                                           format=format,
                                           image_url=f'gs://{parsed_url["bucket_name"]}/{blob.name}',
                                           status=SourceFileStatus.FOUND))
                            session.commit()
                else:
