...
try:
    import gxiba
    from gxiba.environment import BandMetadata, ImageMetadata
...
def update_database_metadata():
    ...
    def get_image_metadata_from_big_query(platform: gxiba.SatelliteImagePlatform | str,
        ...
        for result in gxiba.GXIBA_BIGQUERY_INTERFACE.query(query_constructor_buffer):
            yield ImageMetadata(*result)

    for location in gxiba.GXIBA_LOCATIONS:
        for key in location:
            logger.debug(key)
            for image_metadata in get_image_metadata_from_big_query(platform=gxiba.SatelliteImagePlatform.SENTINEL_2,
                        ...
                        if params is not None:
                            ...
                            for band_metadata in params:
                                if band_metadata != 'band':
                                    logger.debug(f'...Adding {band_metadata} from {platform_id}, band {params["band"]} to database.')
                                    BandMetadata(
                                        platform_id=platform_id,
                                        band=params['band'],
